...
import { useNavigate, useParams, Link } from 'react-router-dom';
import { useSelector } from 'react-redux';
import { ArrowLeft, Send, MessageSquare, Package, Edit2, Trash2, X } from 'lucide-react'; 
import { useGetOrdersByEmailQuery } from "../../../redux/features/orders/orderApi";
import { 
    useGetChatByIdQuery, 
...
const CustomerPaymentChat = () => {
    ...
    const { data: ordersData, isLoading: ordersLoading } = useGetOrdersByEmailQuery(user?.email, {
        skip: !user?.email,
    ...
    const { data: dbChat, isLoading: chatLoading } = useGetChatByIdQuery(selectedOrderId, {
    ...
    // 🚀 Update seen status and message count
    useEffect(() => {
        if (selectedOrderId && dbChat?.messages) {
            localStorage.setItem(`last_read_status_${selectedOrderId}`, activeOrder?.status || 'seen');
            localStorage.setItem(`last_msg_count_${selectedOrderId}`, dbChat.messages.length.toString());
            setReadChats(prev => ({ ...prev, [selectedOrderId]: true }));
        }
    }, [selectedOrderId, dbChat?.messages, activeOrder?.status]);

    const initialAdminMessage = { 
        _id: 'instruction-msg-static', 
        sender: 'admin', 
        text: `Hello ${user?.username || 'Customer'}! Please complete your payment of Rs. ${(activeOrder?.amount || activeOrder?.totalAmount || 0).toLocaleString()} using:\n\n• Easypaisa: 03XXXXXXXXX\n• JazzCash: 03XXXXXXXXX\n• Bank Transfer: XXXX-XXXX-XXXX-XXXX\n\nAccount Title: Abdul Wahab\n\nAfter sending, please upload your receipt here.`, 
        createdAt: activeOrder?.createdAt || new Date().toISOString()
    };

    const messages = dbChat?.messages ? [initialAdminMessage, ...dbChat.messages] : [initialAdminMessage];
    const chatStatus = dbChat?.status || activeOrder?.status || 'Pending';
    ...
    const scrollToBottom = (force = false) => {
        if (chatContainerRef.current) {
            ...
            // Check if user is already near bottom (150px threshold)
    ...
    // Auto-scroll when messages change (polling or new msgs)
    useEffect(() => {
        scrollToBottom();
    }, [messages]);
    ...
    const handleDelete = async (msgId) => {
        if (window.confirm("Are you sure? This will remove message and receipt image.")) {
            try {
                await deleteMessage({ orderId: selectedOrderId, messageId: msgId }).unwrap();
    ...
    const handleSendOrUpdate = async (e) => {
        ...
        const processMessage = async (base64Image = null) => {
            const userMsg = { 
                sender: 'user', 
                text: inputText.trim(), 
            ...
            try {
                if (editingMessageId) {
                    await updateMessage({ orderId: selectedOrderId, messageId: editingMessageId, message: userMsg }).unwrap();
                } else {
                    await sendMessageToDb({ id: selectedOrderId, message: userMsg }).unwrap();
                }
                cancelEdit();
                // User sent a message, so force scroll to bottom
        ...
        if (selectedFile) {
            const reader = new FileReader();
            reader.onloadend = () => processMessage(reader.result);
            reader.readAsDataURL(selectedFile);
        } else { processMessage(); }
    };

    if (ordersLoading) return <div className="h-screen flex items-center justify-center font-bold text-indigo-600 animate-pulse">Loading Your Chats...</div>;
        ...
        <div className="max-w-6xl mx-auto h-[85vh] flex flex-col md:flex-row bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-gray-100 mt-4">
            ...
            <div className="w-full md:w-[350px] bg-gray-50 border-r border-gray-200 flex flex-col h-1/3 md:h-full">
                ...
                <div className="flex-1 overflow-y-auto">
                    {manualOrders.map(order => {
                        ...
                        const lastReadStatus = localStorage.getItem(`last_read_status_${order._id}`);
                        const showDot = !isSelected && !readChats[order._id] && 
                                       (order.status !== 'Pending' && lastReadStatus !== order.status);

                        return (
                            <div key={order._id} onClick={() => setSelectedOrderId(order._id)} className={`p-5 border-b border-gray-100 cursor-pointer transition-all flex items-center gap-4 relative ${isSelected ? 'bg-indigo-50 border-l-4 border-l-indigo-600' : 'hover:bg-white'}`}>
                                {showDot && <div className="absolute top-5 right-5 h-2.5 w-2.5 bg-red-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.6)]"></div>}
                                <div className={`p-3 rounded-full ${isSelected ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-500'}`}><Package size={18} /></div>
                                <div className="flex-1">
                                    <h4 className={`text-sm font-bold ${isSelected ? 'text-indigo-900' : 'text-gray-700'}`}>Order #{order._id.slice(-6)}</h4>
                                    <div className="flex justify-between items-center mt-1 text-xs">
                                        <span className="text-gray-500 font-medium">Rs. {(order.amount || order.totalAmount || 0).toLocaleString()}</span>
                                        <span className={`font-bold px-2 py-0.5 rounded-full text-[9px] uppercase ${order.status === 'Approved' ? 'bg-green-100 text-green-700' : order.status === 'Rejected' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}`}>{order.status}</span>
                                    </div>
            ...
            {activeOrder ? (
                <div className="flex-1 flex flex-col h-2/3 md:h-full relative bg-[#F8FAFC]">
                    ...
                    {/* Chat Container with Smart Scroll Ref */}
                    <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth">
                        {chatLoading && <p className="text-center text-[10px] text-indigo-400 font-bold animate-pulse">Syncing...</p>}
                                ...
                                <div className={`max-w-[80%] flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'} group`}>
                                    <div className={`p-4 rounded-3xl shadow-sm text-sm leading-relaxed ${msg.sender === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-indigo-50 border border-indigo-100 text-gray-700 rounded-tl-none'}`}>
                                        {msg.imageUrl && (
                                            <a href={msg.imageUrl} target="_blank" rel="noopener noreferrer">
                                                <img src={msg.imageUrl} alt="Receipt" className="max-w-[200px] rounded-lg mb-2 border border-white/20 cursor-pointer hover:opacity-90" />
                                            </a>
                                        )}
                                        {msg.text && <p className="whitespace-pre-line">{msg.text}</p>}
                                    </div>
                                    {msg.sender === 'user' && (
                                        <div className="flex gap-3 mt-1 text-[10px] font-bold text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity px-1">
                                            <button onClick={() => handleEdit(msg)} className="flex items-center gap-1 hover:text-indigo-600"><Edit2 size={10}/> Edit</button>
                                            <button onClick={() => handleDelete(msg._id)} className="flex items-center gap-1 hover:text-red-500"><Trash2 size={10}/> Delete</button>
                                        </div>
                                    )}
                                    <span className="text-[9px] mt-1 font-bold text-gray-400 px-2 uppercase">
                                        {msg.createdAt ? new Date(msg.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : 'System'}
                                    </span>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="p-6 bg-white border-t border-gray-100 z-10">
                        {editingMessageId && (
                            <div className="flex justify-between items-center bg-amber-50 text-amber-800 text-[10px] px-4 py-2 rounded-t-xl border border-amber-100 border-b-0 font-bold uppercase tracking-wider">
                                <span>Editing Message Mode</span>
                                <button onClick={cancelEdit} className="text-red-500 hover:scale-110"><X size={14}/></button>
                            </div>
                        )}
                        {previewUrl && (
                            <div className="mb-3 relative inline-block border-2 border-dashed border-indigo-300 p-2 rounded-xl bg-indigo-50">
                                <img src={previewUrl} alt="Preview" className="h-24 object-contain rounded-lg shadow-sm" />
                                <button type="button" onClick={() => { setSelectedFile(null); setPreviewUrl(null); }} className="absolute -top-3 -right-3 bg-red-500 text-white w-6 h-6 flex items-center justify-center rounded-full font-bold text-xs">✕</button>
                            </div>
                        )}
                        <form onSubmit={handleSendOrUpdate} className="flex flex-col gap-2">
                            <div className={`flex items-center gap-3 bg-gray-50 p-2 rounded-[2rem] border transition-all ${editingMessageId ? 'border-amber-300 ring-2 ring-amber-100' : 'border-gray-200 focus-within:ring-2 ring-indigo-100'}`}>
                                <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={(e) => setSelectedFile(e.target.files[0])} />
                                <button type="button" onClick={() => fileInputRef.current?.click()} className="p-3 rounded-full text-gray-500 hover:text-indigo-600 transition-colors shadow-sm ml-1">📎</button>
                                <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder={editingMessageId ? "Edit your message..." : "Type your message here..."} className="flex-1 bg-transparent p-2 outline-none text-sm text-gray-700" />
                                <button type="submit" disabled={!inputText.trim() && !selectedFile && !previewUrl} className={`p-4 rounded-full shadow-lg ${(!inputText.trim() && !selectedFile && !previewUrl) ? 'bg-gray-300 text-gray-500' : editingMessageId ? 'bg-amber-500 text-white' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}>
                                    {editingMessageId ? <Edit2 size={20} /> : <Send size={20} />}
                                </button>
                            </div>
                        </form>
                    </div>
